...
A scale is said to have measurement invariance (or measurement equivalence) across groups if subjects with identical levels of the latent construct have the same expected raw scores on the measure [@hirschfeld_multiple-group_2014]. (COMMENT: this isn't citing correctly)
...
My experience with invariance testing is the multiple language/cross-cultural/international context. What makes a test culturally transferable? Byrne's [-@byrne_adaptation_2016] article provides an excellent current and thorough review.  Very surface highlights:
...
In this lesson, we will focus rather narrowly on Byrne's [-@byrne_adaptation_2016] strategy for the statistical/psychometric evaluation of invariance. You might also be interested in Gerstein's [-@gerstein_theory_2021] Systematic Test of Equivalence Procedure (STEP), which walks the researcher, item-by-item, through a six step analysis of the cultural appropriateness of each item. The researcher is prompted to consider why items are and are not appropriate/relevant and how they might be modified. 
...
* Specify and compare a series of increasingly restrictive models. If the fit is unacceptable, stop. Consider investigating the source of partial measurement invariance.
...
* Imposing an equality constraint over groups on the unstandardized coefficient of each indicator. 
...
* Comparing with the Chi-square difference ($\chi_{D}^{2}$) test the configural invariance model and the weak invariance model
    ...
    + Thus, the weak invariance model would be compared to the configural invariance model.  IF we use the $\chi_{D}^{2}$ > .05 (and we'll learn later that there are more and better options), then we can claim weak invariance.
...
* If weak invariance is supported, then we can claim that constructs are manifested the same way in each group
    ...
    + Slopes from regressing the indicators on their respective factors are equal across all groups
...
* Imposing equality constraints on unstandardized pattern coefficients and intercepts.
...
* **Differential item functioning (DIF)** is the pattern that an indicator has appreciably unequal pattern coefficients or intercepts over groups (DIF violates measurement invariance).
...
* Simulation studies suggest stability for different model characteristics such as number of indicators per factor.  Here are some findings (guidelines?) for different testing scenarios:
...
The notion of partial measurement invariance was introduced by Byrne et al. (1989) and is often used to describe an *intermediate* state of invariance.  For example, weak invariance assumes cross-group equality of each unstandardized pattern coefficient.  If some, but not all, pattern coefficients are invariant, then only *partial weak invariance* can be claimed.
...
This lesson's research vignette emerges from Conover et al's [-@conover_development_2017] Ableist Microaggressions Scale (AMS). The article reports on a series of three studies comprised of the development, refinement, and psychometric evaluation of the AMS. We simulate data from the results of the exploratory factor analysis in the second study. 
...
There are 20 (COMMENT: Above it says 22 items) items on the AMS scale. The frequency scaling ranged from 0 (*never*) to 5 (*very frequently*).
...
Let's take a quick peek at the data to see if everything looks correct.
```{r}
...
Our fit is fairly similar to what Conover et al. [-@conover_development_2017] reported in their article. Specifically, their four-factor, correlated factors model had a statisticaly significant Chi-square.  Regarding fit:  CFI = .89, SRMR = .07, and RMSEA = .07 CI 90% (.06, .07). As researchers, they were satisfied with the result and they asked the question, "Is measure invariant across disability severity?"  A first (but not complete) step is to separately evaluate the model for the groups of interest.  In their case, it was mild (where they combined mild and moderate levels of severity) and severe (combining severe and very severe levels).  
...
Let's run the CFA model for those participants whose data was classified as "mild."
```{r}
...
Let's run the CFA model again for those participants whose data was classified as "severe."
...
|$RMSEA\leq .05$ (but definitely < .10)|RMSEA = 0.061, CI 90%(0.056 to 0.066)|Yes   |
...
Plus, a little later in the lecture, we will head into *partial measurement invariance.*

Because we failed here, we would normally not continue.  However, this is a lesson and so we go on!
...
|$RMSEA\leq .05$ (but definitely < .10)|RMSEA = 0.063, CI 90%(0.058 to 0.068)|Yes(ish)   |
...
Conover et al. [-@conover_development_2017] reported that the AMS was invariant at configural and weak invariance (i.e., constraining factor loadings to be equal). At the level of strong invariance (i.e., adding constraints to the intercepts), results, the majority of fit indices remained acceptable (CFI = .90, RMSEA = .06, SRMR = .08). However the chi-square difference and CFI change tests were statistically significant:  $\chi_{D}^{2}(20) = 78.83, p < .01$; $\Delta CFI = -.010$. The Conover et al. [-@conover_development_2017] article did not report further investigation regarding partial measurement invariance.
