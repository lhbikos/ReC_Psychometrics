...
The second option comes from the "the back of the book" where a [chapter](#sims) contains simulated data for all of the examples worked in this volume. Each of these are available for CFA.
...
* **A priori specification of the number of factors**
  + EFA requires no a priori specification; prior to extraction an EFA program will extract as many factors as indicators.  Typically, in subsequent analyses, researchers will specify how many factors to extract.
...
* **Identification status**. The *identification* of a model has to do with whether it is theoretically possible for a computer to derive a unique set of model parameter estimates. Identification is related to model *degrees of freedom* -- later we will explore under-, just-, and over-identified models.  For now...
...
Kline [-@kline_principles_2016] admonishes (COMMENT: I personally had to look this word up as I was unfamilar, and perhaps others may be unfamilar. Maybe more layman's terms would be useful?) us to not overinterpret the labels "exploratory" and "confirmatory".  Why?
...
The image represents the hypothesis that $AS_1 - AS_9$, $AF_1 - AF_4$, $MI_1 - MI_5$, and $AUA_1 - AUA_4$ measure, respectively, the AS, AF, MI, and AUA factors, which are assumed to covary.  Specifically, in this model:
...
SEM (COMMENT: I think SEM was mentioned earlier, but it might be useful to put what it means at least once in the chapter to give people a reminder. I even forgot what it stands for!), in general, requires that all models be *identified.*  Measurement models analyzed in CFA share this requirement, but identification is more straightforward than in other models.  
...
* At least three per factor for a unidimensional model and at least two per factor for a multidimensional model (but more is safer).
* The items/indicators should have reasonable internal consistency, they should correlate with each other, and they should correlate more with themselves than with items on other factors (if multidimensional).
...
* It's really just regression
  + tilde (~, *is regressed on*) is regression operator 
...
This lesson's research vignette emerges from Keum et al's ([@keum_gendered_2018]) Gendered Racial Microaggressions Scale for Asian American Women (GRMSAAW). The article reports on two separate studies that comprised the development, refinement, and psychometric evaluation of two parallel versions (stress appraisal, frequency) of the scale. We simulate data from the final construction of the frequency version as the basis of the lecture.  If the scale looks somewhat familiar it is because the authors used the Gendered Racial Microaggressions Scale for Black Women [@lewis_construction_2015] as a model. 
...
The four factors are their respective number of items and a sample item are as follows:
...
After a quick look at the plot, let's work through the results.  Rosseel's (2019) *lavaan* tutorial is a useful resource in walking through the output.

The *header* is the first few lines of the information. It contains:

* the *lavaan* version number (0.6-9 that I'm using on 10/04/2021)
...
*Parameter estimates* are the last section.
...
The chi-square ($\chi^2$) statistic that evaluates the *exact-fit hypothesis* that there is no difference between the covariances predicted by the model, given the parameter estimates, and the population covariance matrix.  Rejecting the hypothesis says that: 
...
Kline (year?) recommends evaluating the "cor" residuals.  In our output, these seem to be the "cor.bollen" and are near the bottom.  He recommends that residuals > .10 may be possible sources for misfit.  He also indicated that patterns may be helpful (is there an item that has consistently high residuals?).

Kline (year?) also cautions that there are no dependable or trustworthy connections between the size of the residual and the type or degree of model misspecification.  

My first read of our results is that the items in the AS# factor were well-defined. I suspect that a multifactor solution will improve the fit.
...
Recall, Hu and Bentler's (year?) **combination rule** (which is somewhat contested) suggested that the SRMR be interpreted along with the CFI such that: $CFI \geq .95$ and $SRMR \leq .08$.
