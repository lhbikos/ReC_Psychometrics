...
QUESTION for clarity for above: if there were 120 people and 60 pass an item, and 60 fail an item, would you you get the differential information by doing 50 x 50 (because you do it based on the percentage) or would it be 60 x 60 (because you do it based on the number of test takers)? My apologies as I don't know how to submit as just a comment instead of a suggested edit.
...
* The more homogeneous the test, the higher these intercorrelations will be. If all items were perfectly intercorrelated and all were of the .50 difficulty level: 
  ...
  + half of the test takers would obtain perfect scores and the other half zero scores
* Best to select items with a moderate spread of difficulty but whose AVERAGE difficulty level is .50
...
* *internal* criteria serves to maximize internal consistency or homogeneity of the test, example:
  + achievement test, where criteria is total score itself
* *external* criteria serves to maximize the validity of an external criterion, example:
...
A general rule of thumb says that anything greater or less than 1.0 is significantly skewed (QUESTION: does this mean that unless it is 1.0, it is skewed? Since this says anything greater or less than 1.0 is significantly skewed?). A formal z-test can be conducted this way: $z_{skewness}= \frac{S-0}{SE_{skewness}}$ 
