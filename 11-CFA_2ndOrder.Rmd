...
* Determining and tracking the identification status of models, including:
    + nested/nesting models, how those impact fit, and how comparisons are completed relative to nesting status
...
The second option comes from the "the back of the book" where a [chapter](#sims) contains simulated data for all of the examples worked in this volume. Any of these are available for CFA.
...
* Write an APA style results sections with table(s) and figure(s).
...
* Dekay, N. (2021). Quick Reference Guide: The statistics for psychometrics  https://www.humanalysts.com/quick-reference-guide-the-statistics-for-psychometrics
...
* Creating an items-only dataframe where all items are scaled in the same direction (e.g., negatively worded items are reverse-scored).
...
1. Every latent variable (including errors) must be scaled
...
**The bad news** is that SEM/CFA computer tools are not generally helpful in determining whether a model is identified or not.  Why?  Computers are great at numerical processing but not symbolic processing (which iis needed for determining identification status). This means that we, the researchers, must learn the *identification heuristics* to determine the model's degree of identification.
...
* Every latent variable (including errors) must be scaled such that the $(df_{M} \geq 0)$

Operationally, in a standard CFA model:

1. A single factor model needs at least 3 indicators for the single factor.
2. Factor models with more than one factor require two or more indicators per factor.
...
In this lesson, we continue using Keum et al's [@keum_gendered_2018] Gendered Racial Microaggressions Scale for Asian American Women (GRMSAAW). The article reports on separate studies that comprised the development, refinement, and psychometric evaluation of two parallel versions (stress appraisal, frequency) of the GRMSAAW. We simulate data from the final construction of the frequency version as the basis of the lecture. If the scale looks somewhat familiar it is because the authors used the Gendered Racial Microaggressions Scale for Black Women [@lewis_construction_2015] as a model. 
...
* It's really just regression
    + tilde (~, *is regressed on*) is the regression operator 
...
Although this fit is reasonable, the correlated factors model should have a better fit. Instead of "tweaking" this one, let's move on to the correlated factors model.
...
In our example, uncorrF has four uncorrelated factors and the degrees of freedom were 209.
...
Examining the two models side-by-side (uncorrelated on left and correlated on right), we can visualize the additional "sticks" (i.e., the covariances that were freed) in the correlated factors model and guess (without looking) that it because it has (a) fewer degrees of freedom, it will have (b) better fit.
...
If this were my data, I would be happy with these results and not respecify the model. However, because this is a teaching lesson, I will demonstrate the respecification and evaluation. 
...
In the context of this instrument whose CFA properties are already strong, I find it difficult to justify allowing these errors to covary, but I want to demonstrate the technique. We respecify it by adding (COMMENT: this last sentence seems incomplete but I am not sure how to edit it and keep the meaning so I left a comment about ;) haha)
...
We then compared the *corrF* model to the two models below. In these comparisons, *corrF* was nested in each of the lower models which had one parameter freed (the cross-loading on the lower left and the error covariance on the lower right). As is common, each of these nesting models (more parameters, fewer degrees of freedom) had a better fit. However, because the additions were not theoretically justifiable (and the fit for *corrF* and *uncorrF* was satisfatory), we did not retain these respecifications.
...
* two options for scaling *g*
    + fixing the direct of effect of *g* on one factor (usually the first or last) to 1.0 
    OR
...
If we peek back at Figure ?????? (COMMENT: figure ?? hehe), the disturbances in a second-order model resemble the specific factors in a bifactor model in that both sets of variables are independent of *g*.
...
*Although I demonstrated how to obtain and interpret omega hierarchical, total, and subscales, because the results are nonsensical, I refer you to the Keum article for an example of how to write it up.* 
 ...
 (COMMENT: The Chi-square symbol is not loading correctly for some of these in the APA write-up)
