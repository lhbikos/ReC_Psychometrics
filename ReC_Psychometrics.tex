...
\CommentTok{#rm(ObjAlpha, Obj_othR) #It's messay, dropping all the no-longer-necessary objects from the Global Environment}
...
\CommentTok{#rm(MargAlpha, Marg_othR) #It's messay, dropping all the no-longer-necessary objects from the Global Environment}
...
\begin{Highlighting}[]
\CommentTok{#write the simulated data  as a .csv}
\KeywordTok{write.table}\NormalTok{(dfSzyT1, }\DataTypeTok{file=}\StringTok{"dfSzyT1.csv"}\NormalTok{, }\DataTypeTok{sep=}\StringTok{","}\NormalTok{, }\DataTypeTok{col.names=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{row.names=}\OtherTok{FALSE}\NormalTok{)}
\CommentTok{#bring back the simulated dat from a .csv file}
\NormalTok{dfSzyT1 <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{ (}\StringTok{"dfSzyT1.csv"}\NormalTok{, }\DataTypeTok{header =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{TDSCorMat <-}\StringTok{ }\NormalTok{TDSmat }\OperatorTok{%*%}\StringTok{ }\KeywordTok{t}\NormalTok{(TDSmat) }\CommentTok{#create the correlation matrix via some matrix algebra}
\KeywordTok{diag}\NormalTok{(TDSCorMat) <-}\StringTok{ }\DecValTok{1}
\CommentTok{#SzyCorMat #prints the correlation matrix}
\NormalTok{TDS_M <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{3.18}\NormalTok{, }\FloatTok{2.98}\NormalTok{, }\FloatTok{3.22}\NormalTok{, }\FloatTok{3.53}\NormalTok{, }\FloatTok{2.93}\NormalTok{, }\FloatTok{2.54}\NormalTok{, }\FloatTok{4.38}\NormalTok{, }\FloatTok{2.47}\NormalTok{, }\FloatTok{1.47}\NormalTok{, }\FloatTok{1.49}\NormalTok{, }\FloatTok{1.48}\NormalTok{, }\FloatTok{1.86}\NormalTok{, }\FloatTok{1.80}\NormalTok{, }\FloatTok{2.77}\NormalTok{, }\FloatTok{1.49}\NormalTok{, }\FloatTok{1.30}\NormalTok{, }\FloatTok{1.38}\NormalTok{, }\FloatTok{2.35}\NormalTok{, }\FloatTok{1.69}\NormalTok{, }\FloatTok{2.26}\NormalTok{, }\FloatTok{1.63}\NormalTok{) }\CommentTok{#item means from Table 2}
\NormalTok{TDS_SD <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{1.80}\NormalTok{, }\FloatTok{1.46}\NormalTok{, }\FloatTok{1.43}\NormalTok{, }\FloatTok{1.44}\NormalTok{, }\FloatTok{1.28}\NormalTok{, }\FloatTok{1.22}\NormalTok{, }\FloatTok{1.46}\NormalTok{, }\FloatTok{1.41}\NormalTok{, }\FloatTok{.96}\NormalTok{, }\FloatTok{.87}\NormalTok{, }\FloatTok{.86}\NormalTok{, }\FloatTok{1.25}\NormalTok{, }\FloatTok{1.23}\NormalTok{, }\FloatTok{1.75}\NormalTok{, }\FloatTok{.81}\NormalTok{, }\FloatTok{.65}\NormalTok{, }\FloatTok{.71}\NormalTok{, }\FloatTok{1.39}\NormalTok{, }\FloatTok{1.17}\NormalTok{, }\FloatTok{1.45}\NormalTok{, }\FloatTok{.97}\NormalTok{) }\CommentTok{#item standard deviations from Table 2}
\NormalTok{TDSCovMat <-}\StringTok{ }\NormalTok{TDS_SD }\OperatorTok{%*%}\StringTok{ }\KeywordTok{t}\NormalTok{(TDS_SD) }\OperatorTok{*}\StringTok{ }\NormalTok{TDSCorMat }\CommentTok{#creates a covariance matrix (with more matrix algebra) from the correlation matrix}
\NormalTok{dfTDS <-}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{round}\NormalTok{(MASS}\OperatorTok{::}\KeywordTok{mvrnorm}\NormalTok{(}\DataTypeTok{n=}\DecValTok{220}\NormalTok{, }\DataTypeTok{mu =}\NormalTok{ TDS_M, }\DataTypeTok{Sigma =}\NormalTok{ TDSCovMat, }\DataTypeTok{empirical =} \OtherTok{TRUE}\NormalTok{),}\DecValTok{0}\NormalTok{)) }\CommentTok{#creates the item level data from the sample size, mean, and covariance matrix; wrapped in commands to round to 0 decimal places and format as a df}
\NormalTok{dfTDS[dfTDS}\OperatorTok{>}\DecValTok{6}\NormalTok{]<-}\DecValTok{6} \CommentTok{#restricts the upperbound of all variables to be 6 or less}
\NormalTok{dfTDS[dfTDS}\OperatorTok{<}\DecValTok{1}\NormalTok{]<-}\DecValTok{1} \CommentTok{#resticts the lowerbound of all variable to be 0 or greater}
\CommentTok{#colMeans(GRMS) #displays column means}

\CommentTok{#Below is code if you would like an ID number for each case. Expecially at first, the ID number would just need to be removed, so I will not include it in the original simulation. We will add it later.}
\CommentTok{#library(tidyverse)}
\CommentTok{#dfTDS <- dfTDS %>% dplyr::mutate(ID = row_number()) #add ID to each row}
\CommentTok{#dfTDS <- dfTDS%>%dplyr::select(ID, everything())#moving the ID number to the first column; requires}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{TDS_mu <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{2.12}\NormalTok{, }\FloatTok{2.13}\NormalTok{, }\FloatTok{13.98}\NormalTok{, }\FloatTok{23.57}\NormalTok{, }\FloatTok{16.32}\NormalTok{, }\FloatTok{27.97}\NormalTok{, }\FloatTok{8.62}\NormalTok{)}
\NormalTok{TDS_sd <-}\StringTok{ }\KeywordTok{c}\NormalTok{(.}\DecValTok{66}\NormalTok{, }\FloatTok{.76}\NormalTok{, }\FloatTok{4.87}\NormalTok{, }\FloatTok{6.86}\NormalTok{, }\FloatTok{8.50}\NormalTok{, }\FloatTok{8.49}\NormalTok{, }\FloatTok{3.43}\NormalTok{)}
\NormalTok{TDS_r_mat <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\FloatTok{.83}\NormalTok{, }\FloatTok{.19}\NormalTok{, }\FloatTok{.32}\NormalTok{, }\FloatTok{.13}\NormalTok{, }\FloatTok{.29}\NormalTok{, }\FloatTok{.26}\NormalTok{,}
                  \FloatTok{.83}\NormalTok{,  }\DecValTok{1}\NormalTok{,  }\FloatTok{.20}\NormalTok{, }\FloatTok{.32}\NormalTok{, }\FloatTok{.11}\NormalTok{, }\FloatTok{.33}\NormalTok{, }\FloatTok{.28}\NormalTok{,}
                  \FloatTok{.19}\NormalTok{, }\FloatTok{.20}\NormalTok{,  }\DecValTok{1}\NormalTok{,  }\FloatTok{.38}\NormalTok{, }\FloatTok{.41}\NormalTok{, }\FloatTok{.19}\NormalTok{, }\FloatTok{.12}\NormalTok{,}
                  \FloatTok{.32}\NormalTok{, }\FloatTok{.32}\NormalTok{, }\FloatTok{.38}\NormalTok{,  }\DecValTok{1}\NormalTok{,  }\FloatTok{.46}\NormalTok{, }\FloatTok{.36}\NormalTok{, }\FloatTok{.36}\NormalTok{,}
                 \FloatTok{.13}\NormalTok{, }\FloatTok{.11}\NormalTok{, }\FloatTok{.41}\NormalTok{, }\FloatTok{.46}\NormalTok{, }\DecValTok{1}\NormalTok{,  }\FloatTok{.46}\NormalTok{, }\FloatTok{.35}\NormalTok{,}
                 \FloatTok{.29}\NormalTok{, }\FloatTok{.33}\NormalTok{, }\FloatTok{.19}\NormalTok{, }\FloatTok{.36}\NormalTok{, }\FloatTok{.46}\NormalTok{, }\DecValTok{1}\NormalTok{, }\FloatTok{.35}\NormalTok{,}
                  \FloatTok{.26}\NormalTok{, }\FloatTok{.28}\NormalTok{, }\FloatTok{.12}\NormalTok{, }\FloatTok{.36}\NormalTok{, }\FloatTok{.35}\NormalTok{, }\FloatTok{.72}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{ncol =} \DecValTok{7}\NormalTok{)}
\NormalTok{TDS_cov_mat <-}\StringTok{ }\NormalTok{TDS_sd }\OperatorTok{%*%}\StringTok{ }\KeywordTok{t}\NormalTok{(TDS_sd) }\OperatorTok{*}\StringTok{ }\NormalTok{TDS_r_mat}

\KeywordTok{set.seed}\NormalTok{(}\DecValTok{210927}\NormalTok{)}
\NormalTok{tdsDF <-}\StringTok{ }\KeywordTok{round}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(MASS}\OperatorTok{::}\KeywordTok{mvrnorm}\NormalTok{(}\DataTypeTok{n =} \DecValTok{369}\NormalTok{, }\DataTypeTok{mu=}\NormalTok{TDS_mu, }\DataTypeTok{Sigma=}\NormalTok{TDS_cov_mat, }\DataTypeTok{tol=}\FloatTok{1e-3}\NormalTok{, }\DataTypeTok{empirical=}\OtherTok{TRUE}\NormalTok{)),}\DecValTok{2}\NormalTok{) }\CommentTok{#adding "tol=1e-3" fixed the not positive matrix error}
\NormalTok{tdsDF <-}\StringTok{ }\KeywordTok{round}\NormalTok{(dplyr}\OperatorTok{::}\KeywordTok{rename}\NormalTok{(tdsDF, }\DataTypeTok{TDS21 =}\NormalTok{ V1, }\DataTypeTok{HHRDS =}\NormalTok{ V2, }\DataTypeTok{Nondisclosure =}\NormalTok{ V3, }\DataTypeTok{NegExp =}\NormalTok{ V4, }\DataTypeTok{Intern =}\NormalTok{ V5, }\DataTypeTok{Distress =}\NormalTok{ V6, }\DataTypeTok{Stress =}\NormalTok{ V7),}\DecValTok{2}\NormalTok{)}
\CommentTok{#round(cor(tdsDF),2) #Checking the work}
\end{Highlighting}
\end{Shaded}

\hypertarget{ableist-microaggression-scale}{%
\section{Ableist Microaggression Scale}\label{ableist-microaggression-scale}}

\hypertarget{simulating-item-level-data-2}{%
\subsection{Simulating Item Level Data}\label{simulating-item-level-data-2}}
...
\NormalTok{AMSCorMat <-}\StringTok{ }\NormalTok{AMSmat }\OperatorTok{%*%}\StringTok{ }\KeywordTok{t}\NormalTok{(AMSmat) }\CommentTok{#create the correlation matrix via some matrix algebra}
...
\NormalTok{AMSCovMat <-}\StringTok{ }\NormalTok{AMS_SD }\OperatorTok{%*%}\StringTok{ }\KeywordTok{t}\NormalTok{(AMS_SD) }\OperatorTok{*}\StringTok{ }\NormalTok{AMSCorMat }\CommentTok{#creates a covariance matrix (with more matrix algebra) from the correlation matrix}
\NormalTok{dfAMS <-}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{round}\NormalTok{(MASS}\OperatorTok{::}\KeywordTok{mvrnorm}\NormalTok{(}\DataTypeTok{n=}\DecValTok{559}\NormalTok{, }\DataTypeTok{mu =}\NormalTok{ AMS_M, }\DataTypeTok{Sigma =}\NormalTok{ AMSCovMat, }\DataTypeTok{empirical =} \OtherTok{TRUE}\NormalTok{),}\DecValTok{0}\NormalTok{)) }\CommentTok{#creates the item level data from the sample size, mean, and covariance matrix; wrapped in commands to round to 0 decimal places and format as a df}
\NormalTok{dfAMS[dfAMS}\OperatorTok{>}\DecValTok{5}\NormalTok{]<-}\DecValTok{5} \CommentTok{#restricts the upperbound of all variables to be 5 or less}
\NormalTok{dfAMS[dfAMS}\OperatorTok{<}\DecValTok{0}\NormalTok{]<-}\DecValTok{0} \CommentTok{#resticts the lowerbound of all variable to be 0 or greater}

\CommentTok{#Below is code if you would like an ID number for each case. Expecially at first, the ID number would just need to be removed, so I will not include it in the original simulation. We will add it later.}
\CommentTok{#library(tidyverse)}
\CommentTok{#dfAMS <- dfAMS %>% dplyr::mutate(ID = row_number()) #add ID to each row}
\CommentTok{#dfAMS <- dfAMS %>%dplyr::select(ID, everything())#moving the ID number to the first column; requires}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#quick check to see if it works}
\CommentTok{#amsPAF1 <- psych::fa(dfAMS, nfactors = 4, fm = "pa", max.iter = 100, rotate = "varimax")# using raw data and specifying the max number of factors}
\CommentTok{#amsPAF1}
\end{Highlighting}
\end{Shaded}

\hypertarget{simulating-scale-level-data-for-validity-coefficients-2}{%
\subsection{Simulating Scale Level Data (for Validity Coefficients)}\label{simulating-scale-level-data-for-validity-coefficients-2}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{AMS_mu <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{1.96}\NormalTok{, }\FloatTok{2.76}\NormalTok{, }\FloatTok{1.51}\NormalTok{, }\FloatTok{1.17}\NormalTok{, }\FloatTok{1.69}\NormalTok{, }\FloatTok{1.93}\NormalTok{, }\FloatTok{1.26}\NormalTok{, }\FloatTok{1.21}\NormalTok{)}
\NormalTok{AMS_sd <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{1.18}\NormalTok{, }\FloatTok{1.34}\NormalTok{, }\FloatTok{1.28}\NormalTok{, }\FloatTok{1.03}\NormalTok{, }\FloatTok{.96}\NormalTok{, }\FloatTok{1.08}\NormalTok{, }\FloatTok{.99}\NormalTok{, }\FloatTok{.40}\NormalTok{)}
\NormalTok{AMS_r_mat <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\FloatTok{.27}\NormalTok{, }\FloatTok{.66}\NormalTok{, }\FloatTok{.68}\NormalTok{, }\FloatTok{.84}\NormalTok{, }\FloatTok{.11}\NormalTok{, }\FloatTok{.14}\NormalTok{, }\FloatTok{-.08}\NormalTok{,}
                  \FloatTok{.27}\NormalTok{,  }\DecValTok{1}\NormalTok{,  }\FloatTok{.36}\NormalTok{, }\FloatTok{.30}\NormalTok{, }\FloatTok{.52}\NormalTok{, }\FloatTok{.28}\NormalTok{, }\FloatTok{.34}\NormalTok{, }\FloatTok{-.09}\NormalTok{,}
                  \FloatTok{.66}\NormalTok{, }\FloatTok{.36}\NormalTok{,  }\DecValTok{1}\NormalTok{,  }\FloatTok{.76}\NormalTok{, }\FloatTok{.89}\NormalTok{, }\FloatTok{.22}\NormalTok{, }\FloatTok{.25}\NormalTok{, }\FloatTok{-.09}\NormalTok{,}
                  \FloatTok{.68}\NormalTok{, }\FloatTok{.30}\NormalTok{, }\FloatTok{.76}\NormalTok{,  }\DecValTok{1}\NormalTok{,  }\FloatTok{.90}\NormalTok{, }\FloatTok{.21}\NormalTok{, }\FloatTok{.24}\NormalTok{, }\FloatTok{-.15}\NormalTok{,}
                 \FloatTok{.84}\NormalTok{, }\FloatTok{.52}\NormalTok{, }\FloatTok{.89}\NormalTok{, }\FloatTok{.90}\NormalTok{, }\DecValTok{1}\NormalTok{,  }\FloatTok{.24}\NormalTok{, }\FloatTok{.29}\NormalTok{, }\FloatTok{.13}\NormalTok{,}
                 \FloatTok{.11}\NormalTok{, }\FloatTok{.28}\NormalTok{, }\FloatTok{.22}\NormalTok{, }\FloatTok{.21}\NormalTok{, }\FloatTok{.24}\NormalTok{,  }\DecValTok{1}\NormalTok{, }\FloatTok{.75}\NormalTok{, }\FloatTok{-.16}\NormalTok{,}
                  \FloatTok{.14}\NormalTok{, }\FloatTok{.34}\NormalTok{, }\FloatTok{.25}\NormalTok{, }\FloatTok{.24}\NormalTok{, }\FloatTok{.29}\NormalTok{, }\FloatTok{.75}\NormalTok{,  }\DecValTok{1}\NormalTok{, }\FloatTok{-.16}\NormalTok{,}
                 \FloatTok{-.08}\NormalTok{, }\FloatTok{-.09}\NormalTok{, }\FloatTok{-.09}\NormalTok{, }\FloatTok{-.15}\NormalTok{, }\FloatTok{-.13}\NormalTok{, }\FloatTok{-.16}\NormalTok{, }\FloatTok{-.13}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{ncol =} \DecValTok{8}\NormalTok{)}
\NormalTok{AMS_cov_mat <-}\StringTok{ }\NormalTok{AMS_sd }\OperatorTok{%*%}\StringTok{ }\KeywordTok{t}\NormalTok{(AMS_sd) }\OperatorTok{*}\StringTok{ }\NormalTok{AMS_r_mat}

\KeywordTok{set.seed}\NormalTok{(}\DecValTok{210927}\NormalTok{)}
\NormalTok{amsDF <-}\StringTok{ }\KeywordTok{round}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(MASS}\OperatorTok{::}\KeywordTok{mvrnorm}\NormalTok{(}\DataTypeTok{n =} \DecValTok{833}\NormalTok{, }\DataTypeTok{mu=}\NormalTok{AMS_mu, }\DataTypeTok{Sigma=}\NormalTok{AMS_cov_mat, }\DataTypeTok{tol=}\FloatTok{1e-2}\NormalTok{, }\DataTypeTok{empirical=}\OtherTok{TRUE}\NormalTok{)),}\DecValTok{2}\NormalTok{) }\CommentTok{#adding "tol=1e-#" fixed the not positive matrix error -- but had to wiggle it around until a number (2) worked}
\NormalTok{amsDF <-}\StringTok{ }\KeywordTok{round}\NormalTok{(dplyr}\OperatorTok{::}\KeywordTok{rename}\NormalTok{(amsDF, }\DataTypeTok{Helplessness =}\NormalTok{ V1, }\DataTypeTok{Minimization =}\NormalTok{ V2, }\DataTypeTok{Personhood =}\NormalTok{ V3, }\DataTypeTok{Otherization =}\NormalTok{ V4, }\DataTypeTok{AMS =}\NormalTok{ V5, }\DataTypeTok{PSS4 =}\NormalTok{ V6, }\DataTypeTok{CESD10 =}\NormalTok{ V7, }\DataTypeTok{SDRS5 =}\NormalTok{ V8),}\DecValTok{2}\NormalTok{)}
\CommentTok{#round(cor(tdsDF),2) #Checking the work}
\end{Highlighting}
\end{Shaded}

\hypertarget{gendered-racial-microaggressions-scale-for-black-women-stress-appraisal}{%
\section{Gendered Racial Microaggressions Scale for Black Women (Stress Appraisal)}\label{gendered-racial-microaggressions-scale-for-black-women-stress-appraisal}}

The GRMS has two scales: frequency and stress appraisal. This simulation is for the stress appraisal scale.

\hypertarget{simulating-item-level-data-3}{%
\subsection{Simulating Item Level Data}\label{simulating-item-level-data-3}}
...
\NormalTok{GRMSAAWCorMat <-}\StringTok{ }\NormalTok{GRMSAAWmat }\OperatorTok{%*%}\StringTok{ }\KeywordTok{t}\NormalTok{(GRMSAAWmat) }\CommentTok{#create the correlation matrix via some matrix algebra}
...
\NormalTok{GRMSAAWCovMat <-}\StringTok{ }\NormalTok{GRMSAAW_SD }\OperatorTok{%*%}\StringTok{ }\KeywordTok{t}\NormalTok{(GRMSAAW_SD) }\OperatorTok{*}\StringTok{ }\NormalTok{GRMSAAWCorMat }\CommentTok{#creates a covariance matrix (with more matrix algebra) from the correlation matrix}
\NormalTok{dfGRMSAAW <-}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{round}\NormalTok{(MASS}\OperatorTok{::}\KeywordTok{mvrnorm}\NormalTok{(}\DataTypeTok{n=}\DecValTok{304}\NormalTok{, }\DataTypeTok{mu =}\NormalTok{ GRMSAAW_M, }\DataTypeTok{Sigma =}\NormalTok{ GRMSAAWCovMat,  }\DataTypeTok{empirical =} \OtherTok{TRUE}\NormalTok{),}\DecValTok{0}\NormalTok{)) }\CommentTok{#creates the item level data from the sample size, mean, and covariance matrix; wrapped in commands to round to 0 decimal places and format as a df}
\NormalTok{dfGRMSAAW[dfGRMSAAW}\OperatorTok{>}\DecValTok{5}\NormalTok{]<-}\DecValTok{5} \CommentTok{#restricts the upperbound of all variables to be 5 or less}
\NormalTok{dfGRMSAAW[dfGRMSAAW}\OperatorTok{<}\DecValTok{0}\NormalTok{]<-}\DecValTok{0} \CommentTok{#resticts the lowerbound of all variable to be 0 or greater}

\CommentTok{#Below is code if you would like an ID number for each case. Expecially at first, the ID number would just need to be removed, so I will not include it in the original simulation. We will add it later.}
\CommentTok{#library(tidyverse)}
\CommentTok{#dfGRMSAAW <- dfGRMSAAW %>% dplyr::mutate(ID = row_number()) #add ID to each row}
\CommentTok{#dfGRMSAAW <- dfGRMSAAW %>%dplyr::select(ID, everything())#moving the ID number to the first column; requires}
\end{Highlighting}
\end{Shaded}

\hypertarget{simulating-scale-level-data-for-validity-coefficients-3}{%
\subsection{Simulating Scale Level Data (for Validity Coefficients)}\label{simulating-scale-level-data-for-validity-coefficients-3}}
...
\NormalTok{GRMSAAW_r_mat <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\FloatTok{.00}\NormalTok{, }\FloatTok{.00}\NormalTok{, }\FloatTok{.00}\NormalTok{, }\FloatTok{.00}\NormalTok{, }\FloatTok{.28}\NormalTok{, }\FloatTok{.24}\NormalTok{, }\FloatTok{.46}\NormalTok{, }\FloatTok{.16}\NormalTok{, }\FloatTok{.4}\NormalTok{, }\FloatTok{.29}\NormalTok{, }\FloatTok{.15}\NormalTok{, }\FloatTok{.13}\NormalTok{,}
                          ...
                          \FloatTok{.15}\NormalTok{, }\FloatTok{-.04}\NormalTok{, }\FloatTok{.02}\NormalTok{, }\FloatTok{-.05}\NormalTok{, }\FloatTok{.31}\NormalTok{, }\FloatTok{.10}\NormalTok{, }\FloatTok{.30}\NormalTok{, }\FloatTok{.32}\NormalTok{, }\FloatTok{.11}\NormalTok{, }\FloatTok{.3}\NormalTok{, }\FloatTok{.3}\NormalTok{, }\DecValTok{1}\NormalTok{,.}\DecValTok{18}\NormalTok{,}
                          \FloatTok{.13}\NormalTok{, }\FloatTok{.21}\NormalTok{, }\FloatTok{.17}\NormalTok{, }\FloatTok{-.03}\NormalTok{, }\FloatTok{-.06}\NormalTok{, }\FloatTok{.02}\NormalTok{, }\FloatTok{.16}\NormalTok{, }\FloatTok{.23}\NormalTok{, }\FloatTok{.07}\NormalTok{, }\FloatTok{.14}\NormalTok{, }\FloatTok{.20}\NormalTok{, }\FloatTok{.18}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{ncol =} \DecValTok{13}\NormalTok{)}
\NormalTok{GRMSAAW_cov_mat <-}\StringTok{ }\NormalTok{GRMSAAW_sd }\OperatorTok{%*%}\StringTok{ }\KeywordTok{t}\NormalTok{(GRMSAAW_sd) }\OperatorTok{*}\StringTok{ }\NormalTok{GRMSAAW_r_mat}

\KeywordTok{set.seed}\NormalTok{(}\DecValTok{210927}\NormalTok{)}
\NormalTok{grmsaawDF <-}\StringTok{ }\KeywordTok{round}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(MASS}\OperatorTok{::}\KeywordTok{mvrnorm}\NormalTok{(}\DataTypeTok{n =} \DecValTok{260}\NormalTok{, }\DataTypeTok{mu=}\NormalTok{GRMSAAW_mu, }\DataTypeTok{Sigma=}\NormalTok{GRMSAAW_cov_mat, }\DataTypeTok{tol=}\FloatTok{1e-3}\NormalTok{, }\DataTypeTok{empirical=}\OtherTok{TRUE}\NormalTok{)),}\DecValTok{2}\NormalTok{) }\CommentTok{#adding "tol=1e-#" fixed the not positive matrix error -- but had to wiggle it around until a number (2) worked}
\NormalTok{grmsaawDF <-}\StringTok{ }\KeywordTok{round}\NormalTok{(dplyr}\OperatorTok{::}\KeywordTok{rename}\NormalTok{(grmsaawDF, }\DataTypeTok{AS =}\NormalTok{ V1, }\DataTypeTok{AF =}\NormalTok{ V2, }\DataTypeTok{AUA =}\NormalTok{ V3, }\DataTypeTok{MI =}\NormalTok{ V4, }\DataTypeTok{General =}\NormalTok{ V5, }\DataTypeTok{RMAS_F =}\NormalTok{ V6, }\DataTypeTok{RMAS_L =}\NormalTok{ V7, }\DataTypeTok{RMAS_I =}\NormalTok{ V8, }\DataTypeTok{RMAS_E =}\NormalTok{ V9, }\DataTypeTok{SSE_L =}\NormalTok{ V10, }\DataTypeTok{SSE_R =}\NormalTok{ V11, }\DataTypeTok{PHQ9 =}\NormalTok{ V12, }\DataTypeTok{IRAAS =}\NormalTok{ V13),}\DecValTok{2}\NormalTok{)}
\CommentTok{#round(cor(tdsDF),2) #Checking the work}
\end{Highlighting}
\end{Shaded}
