...
#pca1 <- psych::principal(items, nfactors=25, rotate="none") #using raw data and specifying # factors
...
items$GRMStot <- sjstats::mean_n(items[, GRMSVars], .80)#will create the mean for each individual if 80% of variables are present 
items$Objectified <- sjstats::mean_n(items[,ObjectifiedVars], .80)#will create the mean for each individual if 80% of variables are present 
items$Marginalized <- sjstats::mean_n(items[,MarginalizedVars], .80)#will create the mean for each individual if 80% of variables are present 
items$Strong <- sjstats::mean_n(items[,StrongVars], .80)#will create the mean for each individual if 80% of variables are present (in this case all variables must be present)
items$Angry <- sjstats::mean_n(items[,AngryVars], .80)#will create the mean for each individual if 80% of variables are present (in this case all variables must be present)
```
...
```{r tidy=TRUE, tidy.opts=list(width.cutoff=70)}
Obj_othR <- psych::corr.test(items[c("Obj1", "Obj2", "Obj3", "Obj4", "Obj5", "Obj6", "Obj7", "Obj8", "Obj9", "Obj10", "Marginalized", "Strong", "Angry")])
```

```{r tidy=TRUE, tidy.opts=list(width.cutoff=70)}
Marg_othR <- psych::corr.test(items[c("Marg1", "Marg2", "Marg3", "Marg4", "Marg5", "Marg6", "Marg7", "Objectified", "Strong", "Angry")])
```

```{r tidy=TRUE, tidy.opts=list(width.cutoff=70)}
Str_othR <- psych::corr.test(items[c("Str1", "Str2", "Str3", "Str4", "Str5", "Objectified", "Marginalized", "Angry")])
```

```{r tidy=TRUE, tidy.opts=list(width.cutoff=70)}
Ang_othR <- psych::corr.test(items[c("Ang1", "Ang2", "Ang3",  "Objectified", "Marginalized", "Strong")])
```
...
```{r, child= 'Worked_Examples/4-woRked_PCA.Rmd'}
```
