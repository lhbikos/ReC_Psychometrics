...
There is about two hours of lecture.  If you work through the materials with me, I would plan for an additional hour-and-a-half.
...
* 1,000 = excellent, 300 = good, 100 = poor (Comrey & Lee, 1992)
...
The final column, *com*, represents *item complexity.*  This is an indication of how well an item reflects a single construct. If it is 1.0 then the item loads only on one component, if it's 2.0, it loads evenly on two components, and so forth. For now, we can ignore this. *I mostly wanted to reassure you that "com" is not "communality"... h2 is communality*.
...
*Communality* is the proportion of common variance within a variable.  Principal components analysis assumes that all variance is commo. Before extraction, all variance was set at 1.0 therefore, changing from 25 to 4 components will change this value ($h2$) as well as its associated *uniqueness* ($u2$), which is calculated as "1.0 minus the communality." 
...
Analyzing the residuals means we need to extract only the upper right of the triangle them into an object (NOTE: this sentence is confusing to me, but I am not sure how to reword it so it accurately expresses what you were trying to say here, so I figured I would leave a note. :)). We can do this in steps.
...
Whereas the orthogonal rotation sought to maximize the independence/unrelatedness of the components, an oblique rotation will allow them to be correlated. Researchers often explore both solutions but only report one.
...
pcaOBL_table
```
The same four components/scales seemed to have emerged but they are in different order.  
